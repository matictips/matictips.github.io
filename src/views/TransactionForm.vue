...
export default {
  ...
  methods: {
    ...
    async approveToTipjar()
    {
      ...
      tokenContract.methods.approve(tipjarContractAddress,amt).send({from: userAddress })
    ...
    async depositToTipjar()
    {
      ...
      tipjarContract.methods.depositTokens(tokenAddress,amt).send({from: userAddress })
      .then(function(receipt){
        console.log(receipt)
          // receipt can also be a new contract instance, when coming from a "contract.deploy({...}).send()"
      });
    ...
    async withdrawFromTipjar()
    {
      ...
      tipjarContract.methods.withdrawTokens(tokenAddress,amt).send({from: userAddress})
      .then(function(receipt){
          // receipt can also be a new contract instance, when coming from a "contract.deploy({...}).send()"
      });
